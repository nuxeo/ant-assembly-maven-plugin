...
/**
 * Prints the graph while traversing it. Do not print nodes provided through {@link #addIgnores(Collection)}. Inspired
 * from {@link org.eclipse.aether.util.graph.visitor.AbstractDepthFirstNodeListGenerator}
 *
 * @since 2.0
 */
...
public abstract class AbstractDependencyVisitor implements DependencyVisitor {
    ...
    public boolean visitEnter(DependencyNode node) {
        ...
        if (dependency == null) {
            ...
            AntClient.getInstance().log(String.format("Found node %s with null scope!", node), Project.MSG_DEBUG);
        } else if (scopes != null && !scopes.contains(dependency.getScope())) {
            AntClient.getInstance().log(
                    String.format("Ignored node %s which scope is %s", node, dependency.getScope()), Project.MSG_DEBUG);
            ignoreNode = true;
        }
        if (ignores != null && ignores.contains(node)) {
            AntClient.getInstance().log("Ignored node as requested: " + node, Project.MSG_DEBUG);
            ignoreNode = true;
        }
        if (ignoreNode) {
            visitChildren = false;
            ...
            if (winner != null && !ArtifactIdUtils.equalsId(node.getArtifact(), winner.getArtifact())) {
    ...
    /**
     * Actions to perform when visiting a node. The method is not called if the node was "ignored" (ie included in
     * {@link #ignores}.
     *
    ...
    /**
     * Note that method is always called, even if {@link #doVisit(DependencyNode, boolean)} was not. Check
     * {@link #ignores} if needed.
     */
    @Override
    ...
    public List<Artifact> getArtifacts(boolean includeUnresolved) {
        ...
        for (DependencyNode node : getNodes()) {
            ...
            if (artifact != null && (includeUnresolved || artifact.getFile() != null)) {
